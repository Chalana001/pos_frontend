...
const axisProps = { tick: { fill: "#64748b", fontSize: 11 }, tickLine: false, axisLine: false };
const tooltipStyle = { borderRadius: 12, border: "1px solid #e2e8f0", boxShadow: "0 12px 30px rgb(15 23 42 / 0.12)" };
...
function Metric({ title, value, helper, icon: Icon, accent = "blue" }) {
...
export default function AgingReportView({ data, kind }) {
  ...
  const config = isSupplier ? {
    ...
    chartTitle: "Payables Aging", chartSubtitle: "Outstanding supplier balances by purchase age", tableTitle: "Supplier Payment Priority", color: "#d97706",
    ...
    chartTitle: "Receivables Aging", chartSubtitle: "Outstanding customer credit by invoice age", tableTitle: "Collection Priority", color: "#dc2626",
  ...
  return <div className="space-y-6">
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5"><Metric title={config.totalTitle} value={totals.total} helper={`${rows.length} ${config.countLabel}`} icon={DollarSign} /><Metric title="0-30 Days" value={totals.current} helper={config.recentHelper} icon={Calendar} accent="emerald" /><Metric title="31-60 Days" value={totals.d31} helper={config.middleHelper} icon={Clock} accent="amber" /><Metric title="61-90 Days" value={totals.d61} helper={config.highHelper} icon={AlertCircle} accent="red" /><Metric title="91+ Days" value={totals.d91} helper={config.criticalHelper} icon={AlertCircle} accent="red" /></div>
    <Card className="admin-panel-card border-slate-200/80 p-5"><h2 className="text-lg font-black text-slate-900">{config.chartTitle}</h2><p className="mt-1 text-xs font-semibold text-slate-500">{config.chartSubtitle}</p><div className="mt-5 h-[300px] min-h-[300px] min-w-0">{totals.total > 0 ? <ResponsiveContainer width="100%" height="100%"><BarChart data={buckets} margin={{ top: 12, right: 16, left: 4, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" /><XAxis dataKey="name" {...axisProps} /><YAxis {...axisProps} tickFormatter={shortCurrency} /><Tooltip formatter={(value) => formatCurrency(value)} contentStyle={tooltipStyle} /><Bar dataKey="value" fill={config.color} radius={[8, 8, 0, 0]} /></BarChart></ResponsiveContainer> : <div className="flex h-full items-center justify-center text-sm font-semibold text-slate-400">No outstanding balances</div>}</div></Card>
