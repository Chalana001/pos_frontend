...
export default function GrnPurchaseReportView({ summary }) {
  ...
  return <div className="space-y-6">
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric title="Received Value" value={summary?.totalAmount} helper={`${summary?.page?.totalElements || 0} GRNs`} icon={PackageCheck} tone="accent" /><Metric title="Purchase Returns" value={summary?.totalReturns} helper="Completed supplier returns" icon={RotateCcw} tone="neutral" /><Metric title="Net Received Value" value={summary?.netReceivedAmount} helper="Received less returns" icon={Truck} tone="neutral" /><Metric title="Purchase Payables" value={summary?.totalDue} helper={`${summary?.uniquePurchaseCount || 0} unique purchases`} icon={DollarSign} tone="warning" /></div>
