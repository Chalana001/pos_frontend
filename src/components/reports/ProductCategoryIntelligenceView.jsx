...
export default function ProductCategoryIntelligenceView({ data, intelligenceData, totalElements }) {
  ...
  return <div className="space-y-6">
    ...
    <Card className="admin-panel-card overflow-hidden p-0" title="Product Performance"><div className="flex flex-wrap gap-2 border-b border-slate-200 p-4">{filters.map(f => <button key={f.id} onClick={() => setFilter(f.id)} className={`min-h-11 rounded-full px-3 py-2 text-xs font-bold ${filter === f.id ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-600"}`}>{f.label}</button>)}</div><div className="space-y-3 p-4 md:hidden">{filtered.map(r => <div key={r.itemId} className="rounded-xl border border-slate-200 p-4"><p className="font-black">{r.itemName}</p><p className="mt-1 text-xs text-slate-500">{r.itemType} · {qty(r.qtySold,r.qtyUnit)}</p><div className="mt-3 grid grid-cols-2 gap-3 text-sm"><div><p className="text-xs text-slate-500">Revenue</p><p className="font-bold">{formatCurrency(r.revenue)}</p></div><div><p className="text-xs text-slate-500">Profit / Margin</p><p className={Number(r.profit)<0?"font-bold text-red-700":"font-bold text-emerald-700"}>{formatCurrency(r.profit)} · {Number(r.marginPercent||0).toFixed(1)}%</p></div></div></div>)}</div><div className="hidden md:block"><Table columns={[{header:"Item",render:r=><div><p className="font-bold">{r.itemName}</p><p className="text-xs text-slate-500">{r.itemType}</p></div>},{header:"Qty Sold",render:r=>qty(r.qtySold,r.qtyUnit)},{header:"Revenue",render:r=>formatCurrency(r.revenue)},{header:"Cost",render:r=>formatCurrency(r.cost)},{header:"Profit",render:r=><span className={Number(r.profit)<0?"font-bold text-red-700":"font-bold text-emerald-700"}>{formatCurrency(r.profit)}</span>},{header:"Margin",render:r=>`${Number(r.marginPercent||0).toFixed(1)}%`}]} data={filtered} /></div></Card>
  </div>;
}
