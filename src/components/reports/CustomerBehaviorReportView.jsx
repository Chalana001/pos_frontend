...
export default function CustomerBehaviorReportView({ data }) {
  ...
  return <div className="space-y-6">
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">{metrics.map(([title, value, helper, Icon], index) => <div key={title} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex justify-between gap-3"><div><p className="text-xs font-bold uppercase text-slate-500">{title}</p><p className="mt-2 text-xl font-black text-blue-700">{index === 3 ? `${Number(value || 0).toFixed(1)}%` : index === 4 ? Number(value || 0).toFixed(2) : Number(value || 0).toLocaleString()}</p><p className="mt-1 text-xs font-semibold text-slate-500">{helper}</p></div><Icon size={20} className="text-blue-700" /></div></div>)}</div>
    <Card className="admin-panel-card p-5"><h2 className="text-lg font-black">New vs Returning Customers</h2><div className="mt-4 h-[260px] min-h-[260px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={chart}><XAxis dataKey="name" /><YAxis allowDecimals={false} /><Tooltip /><Bar dataKey="value" fill="#2563eb" radius={[8, 8, 0, 0]} /></BarChart></ResponsiveContainer></div></Card>
