...
import { TILE } from "../../utils/chartTheme";
import useClientPagination from "../../hooks/useClientPagination";
import CustomSelect from "../common/CustomSelect";

const DAY_OPTIONS = [7, 14, 30, 60, 90].map((d) => ({ value: d, label: `${d} days` }));
...
export default function DemandForecastReportView({ summary, accuracy, data, options, lookups, onOptionsChange, onRefresh, onExport, onSchedule }) {
  ...
  const pagination = useClientPagination(filtered, confidence);
  const metrics = [
  ...
  return <div className="space-y-6">
    ...
    <Card className="p-4">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <label className="text-xs font-bold text-slate-600">Forecast horizon<CustomSelect value={options.forecastDays} onChange={(v) => onOptionsChange({ ...options, forecastDays: Number(v) })} options={DAY_OPTIONS} className="mt-1 w-full" buttonClassName="h-10 py-0" /></label>
        <label className="text-xs font-bold text-slate-600">Target cover<CustomSelect value={options.targetCoverDays} onChange={(v) => onOptionsChange({ ...options, targetCoverDays: Number(v) })} options={DAY_OPTIONS} className="mt-1 w-full" buttonClassName="h-10 py-0" /></label>
        <label className="text-xs font-bold text-slate-600">Confidence<CustomSelect value={options.confidence} onChange={(v) => onOptionsChange({ ...options, confidence: v })} options={CONFIDENCE_OPTIONS} className="mt-1 w-full" buttonClassName="h-10 py-0" /></label>
        <label className="text-xs font-bold text-slate-600">Category<CustomSelect value={options.categoryId} onChange={(v) => onOptionsChange({ ...options, categoryId: v ? Number(v) : "" })} options={[{ id: "", name: "All categories" }, ...lookups.categories]} className="mt-1 w-full" buttonClassName="h-10 py-0" /></label>
        <label className="text-xs font-bold text-slate-600">Supplier<CustomSelect value={options.supplierId} onChange={(v) => onOptionsChange({ ...options, supplierId: v ? Number(v) : "" })} options={[{ id: "", name: "All suppliers" }, ...lookups.suppliers]} className="mt-1 w-full" buttonClassName="h-10 py-0" /></label>
    ...
    <Card className="overflow-hidden p-0" title="Item Demand Forecast">
      ...
      <div className="hidden md:block"><Table columns={[
        ...
        { header: "Confidence", render: (row) => <div><ConfidenceBadge value={row.confidence} /><p className="mt-1 text-xs text-slate-500">{row.trend?.replaceAll("_", " ")}</p></div> },
      ...
      ]} data={pagination.pageItems} getRowKey={(row) => row.itemId} /></div>
      <ClientPagination page={pagination.page} pageSize={pagination.pageSize} totalItems={filtered.length} totalPages={pagination.totalPages} onPageChange={pagination.setPage} onPageSizeChange={pagination.setPageSize} />
    </Card>
  </div>;
}
