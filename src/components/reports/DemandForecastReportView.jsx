...
export default function DemandForecastReportView({ summary, data, options, lookups, onOptionsChange, onRefresh, onExport, onSchedule }) {
  ...
  return <div className="space-y-6">
    ...
    <Card className="p-4">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <label className="text-xs font-bold text-slate-600">Category<select className="mt-1 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm" value={options.categoryId} onChange={(event) => onOptionsChange({ ...options, categoryId: event.target.value })}><option value="">All categories</option>{lookups.categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
        <label className="text-xs font-bold text-slate-600">Supplier<select className="mt-1 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm" value={options.supplierId} onChange={(event) => onOptionsChange({ ...options, supplierId: event.target.value })}><option value="">All suppliers</option>{lookups.suppliers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
