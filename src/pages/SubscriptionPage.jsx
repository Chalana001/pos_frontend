...
const SubscriptionPage = () => {
  ...
  // Expire වෙන දවස ලස්සනට හදාගමු (උදා: 2026 March 24)
  const expireDate = new Date(mySubscription?.validUntil).toLocaleDateString('en-US', {
    year: 'numeric', month: 'long', day: 'numeric'
  });

  const isExpired = new Date(mySubscription?.validUntil) < new Date();

  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-5xl mx-auto">
        
        {/* 🚨 Alert Box එක: Expire වෙලාද නැද්ද කියලා පෙන්නන්න */}
        <div className={`p-6 rounded-lg mb-8 shadow-sm border-l-4 ${isExpired ? 'bg-red-50 border-red-500' : 'bg-yellow-50 border-yellow-500'}`}>
          <h2 className={`text-xl font-bold ${isExpired ? 'text-red-700' : 'text-yellow-800'}`}>
            {isExpired ? "⚠️ Your Subscription Has Expired!" : "⏳ Subscription Status"}
          </h2>
          <p className="mt-2 text-gray-700 text-lg">
            Your current package <strong>{mySubscription?.plan?.name}</strong> 
            {isExpired ? " expired on " : " is valid until "} 
            <span className="font-bold text-black">{expireDate}</span>.
          </p>
          <p className="mt-4 text-gray-800 font-medium bg-white p-3 rounded border inline-block">
            ...
            <a href={`https://wa.me/${adminWhatsApp}`} className="text-green-600 font-bold text-xl hover:underline">
              {adminWhatsApp}
            </a>
          </p>
        </div>
          ...
          {plans.map((plan) => {
            ...
            return (
              ...
              >
                ...
                <h4 className="text-xl font-bold text-gray-800 uppercase">{plan.name.replace('_', ' ')}</h4>
                ...
                <div className="my-4">
                  ...
                  {plan.billingCycle === 'MONTHLY' ? <span className="text-gray-500">/mo</span> : <span className="text-gray-500">/lifetime</span>}
                </div>
