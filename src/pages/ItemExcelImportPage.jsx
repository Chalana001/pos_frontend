...
export default function ItemExcelImportPage({ initialTab = "items" }) {
  ...
  const visibleRows = useMemo(() => shownRows.slice(0, visibleRowCount), [shownRows, visibleRowCount]);
  ...
  useEffect(() => {
    ...
    if (!sentinel || !root || visibleRowCount >= shownRows.length) return undefined;

    const observer = new IntersectionObserver(
      (entries) => {
        if (entries.some((entry) => entry.isIntersecting)) {
          setVisibleRowCount((count) => Math.min(count + ROW_PAGE_SIZE, shownRows.length));
        }
      },
  ...
  }, [shownRows.length, visibleRowCount]);
          ...
          */}
          ...
          {shownRows.length === 0 && (
            <p className="py-6 text-center text-sm text-slate-500">
              No rows with this status. Choose All to see the whole file.
            </p>
          )}
